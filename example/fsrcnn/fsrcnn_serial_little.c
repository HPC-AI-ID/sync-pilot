...
static int detect_and_pin_little_core(void) {
#ifdef __linux__
    ...
    int has_freq = 0, num_little = 0;
    int little_ids[n_cpus];
    ...
    if (has_freq && highest_freq > lowest_freq) {
        for (int i = 0; i < n_cpus; i++) {
            if (max_freqs[i] == lowest_freq)
                little_ids[num_little++] = i;
        }
    } else {
        num_little = n_cpus / 2;
        for (int i = 0; i < num_little; i++)
            little_ids[i] = i;
    }

    if (num_little == 0) return -1;

    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    for (int i = 0; i < num_little; i++)
        CPU_SET(little_ids[i], &cpuset);

    sched_setaffinity(0, sizeof(cpu_set_t), &cpuset);

    printf("[SERIAL] Dipin ke %d little core(s):", num_little);
    for (int i = 0; i < num_little; i++) printf(" CPU%d", little_ids[i]);
    printf("\n");
...
int main(int argc, char *argv[]) {
    ...
    mkdir("logs", 0777);
    log_file = fopen("logs/fsrcnn_serial_little.txt", "w");
    ...
    for (int f = 0; f < numFrames; f++) {
        ...
        if (log_file) {
            fprintf(log_file, "[SERIAL-LITTLE] Frame %3d | Layer1: %.5f  Layer2: %.5f  Layer3: %.5f  Layer4: %.5f  Layer5: %.5f  Layer6: %.5f  Layer7: %.5f  Layer8: %.5f  Total: %.5f\n",
    ...
    printf("\n==================================================\n");
    printf("[SERIAL-LITTLE] HASIL WAKTU PER-LAYER FSRCNN\n");
    printf("==================================================\n");
