...
RAW_CSV="${SCRIPT_DIR}/raw_timings_full.csv"
...
for i in "${!SCENARIOS[@]}"; do
    ...
    if [[ "$scen" == *"_NOISE"* ]]; then
        ...
        echo "     [!] Mengaktifkan System Noise Generator dengan ${NOISE_THREADS} Threads..."
...
echo ""
DAT_FILE="${SCRIPT_DIR}/benchmark_data_full.dat"
...
if command -v gnuplot &> /dev/null; then
    ...
    cd "$SCRIPT_DIR"
    gnuplot -e "datafile='${DAT_FILE}'; out_throughput='${SCRIPT_DIR}/fsrcnn_throughput_full.png'; out_time='${SCRIPT_DIR}/fsrcnn_time_full.png'" plot_results.gp
    echo "  [✓] Grafik throughput (fsrcnn_throughput_full.png) dan waktu (fsrcnn_time_full.png) berhasil digenerate!"
else
