...
int main(int argc, char *argv[]) {
    if (argc < 3 || argc > 5) {
        fprintf(stderr, "Usage: %s input.yuv output.yuv [num_workers] [inner_threads]\n", argv[0]);
        return 1;
    }
    ...
    if (argc >= 5) {
        g_inner_threads = atoi(argv[4]);
        if (g_inner_threads < 1) g_inner_threads = 1;
    }

    mkdir("logs", 0777);
    log_file = fopen("logs/fsrcnn_syncpilot_hybrid.txt", "w");
    ...
    printf("Hybrid intra-layer threads: %d\n", g_inner_threads);
...
static void* layer7_worker(void *arg) {
    Layer7WorkerArgs *a = (Layer7WorkerArgs*)arg;
    const int filtersize = 1;
    const int padsize = 0;
    const int num_ch = 12;
    ...
    for (int i = a->start_filter; i < a->end_filter; i++) {
        double *tmp = (double*)malloc(rows * cols * sizeof(double));
        for (int j = 0; j < num_ch; j++) {
            imfilter(a->input + j * rows * cols,
                     weights_layer7 + (i * num_ch + j) * filtersize,
                     tmp, rows, cols, padsize);
            for (int p = 0; p < rows * cols; p++)
                a->output[i * rows * cols + p] += tmp[p];
        }
        PReLU(a->output + i * rows * cols, rows, cols, biases_layer7[i], prelu);
        free(tmp);
    }
...
void layer7(double *input, double *output, int rows, int cols) {
    ...
    int threads = g_inner_threads;
    if (threads > num_filters) threads = num_filters;
    if (threads <= 1) {
        Layer7WorkerArgs args = { input, output, rows, cols, 0, num_filters };
        layer7_worker(&args);
        return;
    }

    pthread_t tids[threads];
    Layer7WorkerArgs args[threads];
    int chunk = (num_filters + threads - 1) / threads;
    for (int t = 0; t < threads; t++) {
        int start = t * chunk;
        int end = start + chunk;
        if (end > num_filters) end = num_filters;
        args[t] = (Layer7WorkerArgs){ input, output, rows, cols, start, end };
        pthread_create(&tids[t], NULL, layer7_worker, &args[t]);
    }
...
static void* layer8_deconv_worker(void *arg) {
    Layer8DeconvArgs *a = (Layer8DeconvArgs*)arg;
    const int filtersize = 81;
    for (int j = a->start_ch; j < a->end_ch; j++) {
        deconv(a->input + j * a->rows * a->cols,
               a->all_tmp + (j * a->hr_pixels),
               weights_layer8 + j * filtersize,
               a->cols, a->rows, a->scale);
    }
    return NULL;
}
...
static void* layer8_reduce_worker(void *arg) {
    Layer8ReduceArgs *a = (Layer8ReduceArgs*)arg;
    for (int p = a->start_pixel; p < a->end_pixel; p++) {
        double sum = 0;
        for (int j = 0; j < a->num_ch; j++) {
            sum += a->all_tmp[j * a->hr_pixels + p];
        }
        a->output[p] = sum + biases_layer8;
    }
    return NULL;
}
...
void layer8(double *input, double *output, int rows, int cols, int scale) {
    ...
    // 2. FASE DECONV
    int threads = g_inner_threads;
    if (threads > num_ch) threads = num_ch;
    if (threads <= 1) {
        Layer8DeconvArgs args = { input, all_tmp, rows, cols, scale, hr_pixels, 0, num_ch };
        layer8_deconv_worker(&args);
    } else {
        pthread_t tids[threads];
        Layer8DeconvArgs args[threads];
        int chunk = (num_ch + threads - 1) / threads;
        for (int t = 0; t < threads; t++) {
            int start = t * chunk;
            int end = start + chunk;
            if (end > num_ch) end = num_ch;
            args[t] = (Layer8DeconvArgs){ input, all_tmp, rows, cols, scale, hr_pixels, start, end };
            pthread_create(&tids[t], NULL, layer8_deconv_worker, &args[t]);
        }
        for (int t = 0; t < threads; t++) pthread_join(tids[t], NULL);
    }
    ...
    if (threads <= 1) {
        Layer8ReduceArgs args = { all_tmp, output, num_ch, hr_pixels, 0, hr_pixels };
        layer8_reduce_worker(&args);
    } else {
        pthread_t tids[threads];
        Layer8ReduceArgs args[threads];
        int chunk = (hr_pixels + threads - 1) / threads;
        for (int t = 0; t < threads; t++) {
            int start = t * chunk;
            int end = start + chunk;
            if (end > hr_pixels) end = hr_pixels;
            args[t] = (Layer8ReduceArgs){ all_tmp, output, num_ch, hr_pixels, start, end };
            pthread_create(&tids[t], NULL, layer8_reduce_worker, &args[t]);
        }
        for (int t = 0; t < threads; t++) pthread_join(tids[t], NULL);
    }
